...
void initMMU() {
    ...
    for (r = 0; r < 512; r++) {
        paging[1 * 512 + r] =
            (unsigned long)((unsigned char *)&paging[(3 + r) * 512]) |     // physical address
    ...
    //(VMSAv8_64_STAGE2_BLOCK_DESCRIPTOR) { .AP = 0, .Address = (uintptr_t)512 << (21 - 12), .AF = 1, .MemAttr =
    // MT_DEVICE_NGNRNE, .EntryType = 1 };
    ...
    for (table = 0; table < 512; table++) {
        for (r = 0; r < 512; r++) {
            address = (table * 512 + r);
            paging[(3 + table) * 512 + r] =
                (unsigned long)(address << 12) |                                       // physical address
                PT_PAGE |                                                              // map 4k
                PT_AF |                                                                // accessed flag
                PT_USER |                                                              // non-privileged
                PT_ISH |                                                               // inner shareable
