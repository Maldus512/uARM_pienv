...
/* Access permission for the Block entries.
    They refer to the permission given to EL0, and EL1/2/3 separately 
    page D4-2157 of the ARM ARM */
    
typedef enum {
	AP_PRIVILEGED = 0,			// Access at EL0 is not permitted. EL1/2/3 can read/write
	AP_UNPRIVILEGED = 1, 	// Access at EL0 is permitted (read/write). EL1/2/3 can read/write
	AP_RESERVED = 2,			// Access at EL0 is not permitted. EL1/2/3 can read only
	AP_READ_ONLY = 3	// Access is always read only
} AP_TYPE;


/* Access Permission for the Page Table entries 
    page D4-2159 of the ARM ARM */
typedef enum {
	APTABLE_NONE = 0,			// No effect in subsequent lookup levels
	APTABLE_PRIVILEGED = 1, 	// Access at EL0 is not permitted (read/write) in subsequent lookup levels
	APTABLE_READ_ONLY = 2,			// Read only at any level in subsequent lookup levels
	APTABLE_PRIVILEGED_READ_ONLY = 3	// Read only at levels EL1/2/3, no access for EL0 in subsequent lookup levels
} APTABLE_TYPE;


/* Table descriptor, pointing to a next level of tables */
typedef union {
	struct __attribute__((__packed__)) 
	{
		uint64_t EntryType : 2;				// @0-1		Always 3 for a page table
		uint64_t _res0 : 10;		// @2-11	Set to 0
        //TODO: Aggiungi un altra union per le varie parti dell'indirizzo
		uint64_t Address : 39;				// @12-50	36 Bits of address
		uint64_t _res1 : 8;		// @51-58	Set to 0
		uint64_t PXNTable : 1;				// @59      Privileged eXecute Never: never allow execution from a higher level than EL0
		uint64_t XNTable : 1;				// @60		eXecute Never: never allow execution from EL0
		APTABLE_TYPE APTable : 2;			// @61-62	Access Permission Table
		uint64_t NSTable : 1;				// @63		For memory access from Secure State
	};
	uint64_t Raw64;							// Raw 64bit access
} VMSAv8_64_NEXTLEVEL_DESCRIPTOR;


/* Block descriptor, pointing to an actual block of memory */
typedef union {
	struct __attribute__((__packed__))
	{
		...
		uint64_t _res0 : 1;			// @11		Set to 0
		uint64_t Address : 39;				// @12-50	36 Bits of address
		uint64_t _res1 : 1;		// @51	Set to 0
		uint64_t Contiguous : 1;			// @52		Contiguous
		uint64_t _res2 : 1;			// @53		Set to 0
		uint64_t XN : 1;					// @54		No execute if bit set
		uint64_t _res3 : 9;		// @55-63	Set to 0
	};
	uint64_t Raw64;							// Raw 64bit access
...
static __attribute__((aligned(4096))) VMSAv8_64_NEXTLEVEL_DESCRIPTOR Stage2map1to1[512*2] = { 0 };

static __attribute__((aligned(4096))) VMSAv8_64_STAGE2_BLOCK_DESCRIPTOR Stage3map1to1[512*512*2] = { 0 };
...
void init_page_table (void) {
	...
	Stage1map1to1[0] = (VMSAv8_64_NEXTLEVEL_DESCRIPTOR) { .NSTable = 0, .Address = (uintptr_t)&Stage2map1to1[0] >> 12, .EntryType = 3 };
	Stage1map1to1[1] = (VMSAv8_64_NEXTLEVEL_DESCRIPTOR) { .NSTable = 0, .Address = (uintptr_t)&Stage2map1to1[512] >> 12, .EntryType = 3 };

	/* The 21-12 entries are because that is only for 4K granual it makes it obvious to change for other granual sizes */

    for (base = 0; base < 512*2; base++) {
        Stage2map1to1[base] = (VMSAv8_64_NEXTLEVEL_DESCRIPTOR) {.Address = (uintptr_t)&Stage3map1to1[base*512] >> 12, .EntryType = 3};
    }

	/* 880Mb of ram */
	for (base = 0; base < 440*512; base++) {
		// Each block descriptor (4 KB)
		Stage3map1to1[base] = (VMSAv8_64_STAGE2_BLOCK_DESCRIPTOR) { .S2AP = 0, .Address = (uintptr_t)base /*<< (21-12)*/, .AF = 1, .SH = STAGE2_SH_INNER_SHAREABLE, 
			                                                        .MemAttr = MT_NORMAL, .EntryType = 3 };
	}

	/* VC ram up to 0x3F000000 */
	for (; base < (512 - 8)*512; base++) {
		// Each block descriptor (4 KB)
		Stage3map1to1[base] = (VMSAv8_64_STAGE2_BLOCK_DESCRIPTOR) {  .S2AP = 0,.Address = (uintptr_t)base /*<< (21 - 12)*/, .AF = 1, .MemAttr = MT_NORMAL_NC, .EntryType = 3 };
	}

	/* 16 MB peripherals at 0x3F000000 - 0x40000000*/
	for (; base < 512*512; base++) {
		// Each block descriptor (4 KB)
		Stage3map1to1[base] = (VMSAv8_64_STAGE2_BLOCK_DESCRIPTOR) {  .S2AP = 0,.Address = (uintptr_t)base /*<< (21 - 12)*/, .AF = 1, .MemAttr = MT_DEVICE_NGNRNE, .EntryType = 3 };
	}
	
	// 2 MB for mailboxes at 0x40000000
	// shared device, never execute
    for (; base < (512+1)*512; base++) {
        Stage3map1to1[base] = (VMSAv8_64_STAGE2_BLOCK_DESCRIPTOR) {  .S2AP = 0,.Address = (uintptr_t)base, .AF = 1, .MemAttr = MT_DEVICE_NGNRNE, .EntryType = 3 };
    }

	// unused up to 0x7FFFFFFF
	for (; base < 1024*512; base++) {
		Stage3map1to1[base].Raw64 = 0;
	}
...
void initMMU(void)
{
	...
	asm volatile("dsb sy"); // Data Synchronization Barrier (System wide). Blocking until all explicit memory accesses are complete
