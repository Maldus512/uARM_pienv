...
typedef enum {
	APTABLE_PRIVILEGED = 0,			// Access at EL0 is not permitted. EL1/2/3 can read/write
	APTABLE_UNPRIVILEGED = 1, 	// Access at EL0 is permitted (read/write). EL1/2/3 can read/write
	APTABLE_RESERVED = 2,			// Access at EL0 is not permitted. EL1/2/3 can read only
	APTABLE_READ_ONLY = 3	// Access is always read only
} APTABLE_TYPE;
...
void init_page_table (void) {
	...
	for (base = 0; base < 440*512; base++) {
		...
		Stage3map1to1[base] = (VMSAv8_64_STAGE2_BLOCK_DESCRIPTOR) { .Address = (uintptr_t)base /*<< (21-12)*/, .AF = 1, .SH = STAGE2_SH_INNER_SHAREABLE, 
	...
	for (; base < (512 - 8)*512; base++) {
		// Each block descriptor (4 KB)
		Stage3map1to1[base] = (VMSAv8_64_STAGE2_BLOCK_DESCRIPTOR) { .Address = (uintptr_t)base /*<< (21 - 12)*/, .AF = 1, .MemAttr = MT_NORMAL_NC, .EntryType = 1 };
	}

	/* 16 MB peripherals at 0x3F000000 - 0x40000000*/
	for (; base < 512*512; base++) {
		// Each block descriptor (4 KB)
		Stage3map1to1[base] = (VMSAv8_64_STAGE2_BLOCK_DESCRIPTOR) { .Address = (uintptr_t)base /*<< (21 - 12)*/, .AF = 1, .MemAttr = MT_DEVICE_NGNRNE, .EntryType = 1 };
	}
	
	// 2 MB for mailboxes at 0x40000000
	// shared device, never execute
    for (; base < (512+1)*512; base++) {
        Stage3map1to1[base] = (VMSAv8_64_STAGE2_BLOCK_DESCRIPTOR) { .Address = (uintptr_t)base, .AF = 1, .MemAttr = MT_DEVICE_NGNRNE, .EntryType = 1 };
    }
